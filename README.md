...
נכסים של צד שלישי שמשולבים בקוד המקור. אין חובה להציג את הקרדיטים האלה
באתר עצמו — הרישיונות דורשים ייחוס בקוד/בהפצה בלבד.

| נכס | שימוש | מקור | רישיון |
|---|---|---|---|
| אייקון „מכירת רכב” (רכב עם תווית מחיר) | סקשן „כל סוגי ההשכרה” ב-`src/components/home/HeroSection.tsx` | [Material Symbols](https://github.com/google/material-design-icons) (Google) | Apache License 2.0 |
| שאר האייקונים בסקשן „כל סוגי ההשכרה” | אותו סקשן | נוצרו לפרויקט הזה | — |
| `lucide-react` | אייקוני ממשק, כולל כפתור הנגישות | [Lucide](https://lucide.dev) | ISC |

אייקון Material Symbols הוא גליף מבוסס **מילוי**: הצבע נקבע ב-`fill` על ה-`path`.
אין להוסיף לו `stroke` או `stroke-width` — זה יתווה כל עקומה ויעבה את הצורה.
