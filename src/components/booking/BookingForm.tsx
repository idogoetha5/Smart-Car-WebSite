...
export default function BookingForm({ vehicle, initialPickupDate = '', initialReturnDate = '', initialLocation = '', initialReturnLocation = '' }: BookingFormProps) {
  ...
  const { subtotal, avgPricePerDay: pricePerDay } =
    pickupDate && dropoffDate
      ? getSeasonalPriceRange(vehicle, pricingConfig, new Date(pickupDate), new Date(dropoffDate))
      : { subtotal: 0, avgPricePerDay: getSeasonalPrice(vehicle, pricingConfig, pickupDate ? new Date(pickupDate) : undefined) };
      ...
      >
        {isSubmitting ? (
          <span className="flex items-center justify-center gap-2">
            <svg aria-hidden="true" focusable="false" className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none">
